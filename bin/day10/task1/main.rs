...
fn get_trailheads(map: &Vec<u8>, index: &usize, max: &Coordinate, trailheads: &mut HashSet<usize>) {
    //println!("{index} ({}): ({}|{})", map[*index], calc_index_x(index, max), calc_index_y(index, max));
    if map[*index] == 9 {
        //println!("    success!");
        trailheads.insert(index.clone());
    }
    for direction in 0 ..4 {
        ...
        get_trailheads(map, &next_index, max, trailheads);
    }
}

fn main() -> std::io::Result<()> {
    println!("Day 10: Task1");
    let is_debug = false;
    let file : File;
    if is_debug {
    ...
    let trailheads: usize = zeros.iter().map(|index| {
        let mut heads: HashSet<usize> = HashSet::new();
        get_trailheads(&map, index, &max, &mut heads);
        heads.len()
    }
    ).sum();
